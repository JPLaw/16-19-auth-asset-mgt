...
imageRouter.get('/api/images/:id?', bearerAuthMiddleware, (request, response, next) => {
  ...
  return Image.findById(request.params.id)
    .then((image) => {
      if (!image) return next(new HttpErrors(404, 'IMAGE ROUTER GET: no sound found'));
      logger.log(logger.INFO, `IMAGE ROUTER GET: successfully found sound ${JSON.stringify(image, null, 2)}`);
